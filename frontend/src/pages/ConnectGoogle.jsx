...
/* ──────────────────────────────────────────────────────────────────────────
   STYLES (Original Beautiful Design)
────────────────────────────────────────────────────────────────────────── */
const CSS = `
...
export default function ConnectGoogle() {
  ...
  useEffect(() => {
    ...
    if (connected === "true" && user) {
      // Connection successful! Check status
  ...
  const checkConnectionStatus = async () => {
    ...
    try {
      ...
      if (data.connected) {
        setBusiness({
          name: data.accountName || 'Your Business',
          address: data.locationName || 'Location verified',
          initials: (data.accountName || 'YB').substring(0, 2).toUpperCase(),
          rating: 4.8,
          reviews: 0
        });
        setState("connected");
              ...
              {(state === "idle" || state === "loading" || state === "error") && (
                <>
                  ...
                  <div className="security-note afu d4">
                    <ShieldIcon />
                    <span>
                      Your Google credentials are never stored. We use OAuth 2.0 — you can revoke access anytime.
                    </span>
                  </div>
